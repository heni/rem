...
class PacketExecutionError(IMessageHelper):
    ...
    def subject(self):
        ...
        return "[REM@%(sname)s] Task '%(pname)s': %(reason)s" % {"pname": self.pck.name, "reason": reason, "sname": self.ctx.network_name} 
...
class PacketExecutionSuccess(IMessageHelper):
    ...
    def subject(self):
        return "[REM@%(sname)s] Task '%(pname)s': successfully executed." % {"pname": self.pck.name, "sname": self.ctx.network_name}
